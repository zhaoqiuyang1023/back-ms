...
<form class="layui-form" style="width:80%;">
    ...
    <input name="id" value="${news.id}" class="layui-input" type="text"  autocomplete="off"
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="title" required  lay-verify="required" autocomplete="off" placeholder="请输入标题"
                   value="${news.title}"
                   class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="summary" required  lay-verify="required" autocomplete="off" placeholder="摘要"
                   value="${news.summary}" class="layui-input">
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <select id="type" name="type" lay-filter="aihao">
                ...
                <#list newsTypes as item>
                    <option <#if item.id==news.type> selected </#if>value="${item.id}">${item.value}</option>
                </#list>
            </select>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="from" required  lay-verify="required"   value="${news.from}" autocomplete="off" placeholder="新闻出处" class="layui-input">
        </div>
    </div>
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-block">
                    <textarea placeholder="请输入内容" name="content" id="content"
                              class="layui-textarea">${news.content}</textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="layui-form-item layui-col-space15" pane>
        <div class="layui-input-block">
            ...
            <input type="radio" name="verifyStatus" value="1"  <#if news.published==true&&news.shelve ==true &&news.recommendable==true  >checked </#if>
                   title="发布并推荐"/>
            <input type="radio" name="verifyStatus" value="2" <#if news.published==true&&news.shelve ==true &&news.recommendable==false>checked </#if>
                   title="发布">

            <input type="radio" name="verifyStatus" value="0"  <#if news.published==false&&news.shelve ==true>checked </#if>
                   title="保存草稿"/>
            <input type="radio" name="verifyStatus" value="3"  <#if news.published==false&&news.shelve ==false>checked </#if>
                   title="已下架"/>
        </div>
    </div>
    <div class="layui-tab-item layui-show">
